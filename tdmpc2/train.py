...
def load_demos(
	...
	for i, task in enumerate(tasks):
		...
		# Update demonstration shape if necessary
		obs_dim = td['obs'].shape[-1]
		is_updated = False
		if obs_dim < expected_obs_dim:
			print(f'Updating demonstration shape ({obs_dim}) for task {task}...')
			td['obs'] = torch.cat([td['obs'], torch.zeros_like(td['obs'])[:, :expected_obs_dim - obs_dim]], dim=-1)
			is_updated = True
		action_dim = td['action'].shape[-1]
		if action_dim < expected_action_dim:
			print(f'[Updating action shape ({action_dim}) for task {task}...')
			td['action'] = torch.cat([td['action'], torch.zeros_like(td['action'])[:, :expected_action_dim - action_dim]], dim=-1)
			is_nan = td['action'][:,0].isnan()
			# replace zeros with nan for actions that were originally nan
			td['action'][is_nan] = float('nan')
			is_updated = True
		elif action_dim > expected_action_dim:
			print(f'Warning: Demonstration action shape ({action_dim}) is larger than expected ({expected_action_dim}) for task {task}. Truncating.')
			td['action'] = td['action'][:, :expected_action_dim]
		if is_updated:
			print(f'Saving updated demonstrations for task {task}...')
			torch.save(td, demo_path)
		...
		if cfg.obs == 'rgb':
			if 'feat' not in td:
				...
				continue
			# if 'feat-stacked' not in td:
			# 	print(f'No visual features found in demonstrations for task {task}, skipping.')
			# 	continue
			# td['obs'] = td['feat-stacked']  # Pretrained visual features
